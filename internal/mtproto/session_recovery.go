...
func (table *PendingTable) recoveryTargets(match func(*PendingRequest) bool) []pendingRecovery {
	if table == nil {
		return nil
	}
	table.mu.Lock()
	targets := make([]pendingRecovery, 0, len(table.entries))
	for _, request := range table.entries {
		if !request.Done && request.message.Body != nil && match(request) {
			targets = append(targets, pendingRecovery{request: request, message: request.message})
		}
	}
	table.mu.Unlock()
	slices.SortFunc(targets, func(left, right pendingRecovery) int {
		switch {
		case left.message.MessageID < right.message.MessageID:
			return -1
		case left.message.MessageID > right.message.MessageID:
			return 1
		default:
...
func (table *PendingTable) reassign(target pendingRecovery, message tl.MTPMessage) bool {
...
func recoverPending(
	...
	targets := pending.recoveryTargets(match)
